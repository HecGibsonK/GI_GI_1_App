...
SysA_process=function(SystemType=NULL){
	...
	if(SystemType=='<Expo_k/Loglogistic(LL)/1>'){
		P_LL = vector('numeric',K);
		A = vector('numeric',K);
		C1 = vector('numeric',K+1);
		Pi_LL = vector('numeric',K+1);
		...
		for(i in 0:(K-1)){
			P_LL[i+1] = integrate(function(x){exp(-L[i+1]*x)*F1(x)},0,Inf)$value
		}
		...
		if(K>=3){
			A[1] = 1;
			A[2] = (1 - (1-(L[2]/(L[2]-L[3])))*P_LL[2])*(1/P_LL[3]);
			
			AK1=0; AK2=0;
			for(j in 1:(K-2)){
				AK1 =AK1 + sum(((-1)^(K-1-j))*(prod(L[(j:(K-2))+1]/(L[j+1]-L[(j:(K-2))+2])))*A[j]);
				AK2 =AK2 + sum(((-1)^(K-j))*(prod(L[(j:(K-2))+2]/(L[j+1]-L[(j:(K-2))+2])))*A[j]*P_LL[j+1]);	
			}
			A[K] = A[K-1]*(1+P_LL[K]) + AK1 - AK2;
		}
		...
		if(K>=4){
			...
			for(i in 2:(K-2)){
				...
				AI2=0;
				for(j in 1:i){
					AI2 =AI2 + sum(((-1)^(i+1-j))*(prod(L[(j:i)+1]/(L[j+1]-L[(j:i)+2])))*A[j]*P_LL[j+1]);
				}
				A[i+1] = (A[i] + AI1 - AI2)*(1/P_LL[i+2]);
				AK1=0; AK2=0;
				for(j in 1:(K-2)){
					AK1 =AK1 + sum(((-1)^(K-1-j))*(prod(L[(j:(K-2))+1]/(L[j+1]-L[(j:(K-2))+2])))*A[j]);
					AK2 =AK2 + sum(((-1)^(K-j))*(prod(L[(j:(K-2))+2]/(L[j+1]-L[(j:(K-2))+2])))*A[j]*P_LL[j+1]);	
		...
		if(K>=3){
			for(i in 2:(K-1)){
				CI1=0;
				for(j in 1:(i-1)){
					CI1 =CI1 + sum(((-1)^(i-j))*(prod(L[(j:(i-1))+1]/(L[j+1]-L[(j:(i-1))+2])))*A[j]*((1-P_LL[j+1])/L[j+1]));	
				}
				C1[i+1] = A[i]*((1-P_LL[i+1])/L[i+1]) + CI1;
			}
		}
		Cn=0;
		if(K==2){
			C1[K+1] = A[K]*b1 - A[K-1]*((1-P_LL[K])/L[K]);
		}
		else{
			for(j in 1:(K-2)){
				Cn =Cn + sum(((-1)^(K-j))*(prod(L[(j:(K-2))+2]/(L[j+1]-L[(j:(K-2))+2])))*A[j]*((1-P_LL[j+1])/L[j+1]));
			}
			C1[K+1] = A[K]*b1 - A[K-1]*((1-P_LL[K])/L[K]) + Cn;
		}
		
		C = (sum(C1))^-1;
		
		Pi_LL[1] = C*(P_LL[2]/L[1]);
		Pi_LL[2] = C*((1 - P_LL[2])/L[2]);
		
		if(K>2){
			for(i in 2:(K-1)){
				Pi=0;
				for(j in 1:(i-1)){
					Pi =Pi + sum(((-1)^(i-j))*(prod(L[(j:(i-1))+1]/(L[j+1]-L[(j:(i-1))+2])))*A[j]*((1-P_LL[j+1])/L[j+1]));
				}
				Pi_LL[i+1] = C*(A[i]*((1-P_LL[i+1])/L[i+1]) + Pi);
			}
		}
		
		Pn=0;
		if(K==2){
			Pi_LL[K+1] = C*(A[K]*b1 - A[K-1]*((1-P_LL[K])/L[K]));
		}
		else{ 
			for(j in 1:(K-2)){
				Pn =Pn + sum(((-1)^(K-j))*(prod(L[(j:(K-2))+2]/(L[j+1]-L[(j:(K-2))+2])))*A[j]*((1-P_LL[j+1])/L[j+1]));
			}
			Pi_LL[K+1] = C*(A[K]*b1 - A[K-1]*((1-P_LL[K])/L[K]) + Pn);
		}
		Pi = Pi_LL;	
	}
